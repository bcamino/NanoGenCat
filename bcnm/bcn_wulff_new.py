...
class Nanoparticle:
    ...
    termNature='non-metal',neutralize=False,inertiaMoment=False,debug=0):
        ...
        if np.sum(nano_remove_array) == 0:
            print('The NP0 is stoichiometric')
            return nano

        nano = make_stoichiometric(nano, nano_remove_array)
...
def remove_undercoordinated_atoms(structure,min_coord):
    """
    ...
    undercoordinated_atoms = np.where(coord_vector <= min_coord)[0]
    
    structure.remove_sites(undercoordinated_atoms)
...
def get_excess_atoms(structure,initial_structure):

    # structure = nanoparticle structure
    # initial_structure = bulk structure

    # returns an array containing the number of atoms to be removed
    
    nano = structure
    
    nano_composition_arr = np.array(list(nano.composition.reduced_composition.as_dict().values()),dtype=int)
    structure_composition_arr = np.array(list(initial_structure.composition.reduced_composition.as_dict().values()),dtype=int)

    if np.all(nano_composition_arr != structure_composition_arr):
        
        ratio_aboundance_np = nano_composition_arr.astype(float)/structure_composition_arr.astype(float)

        less_aboundant = np.min(ratio_aboundance_np)
    
        nano_new_aboundance = less_aboundant*structure_composition_arr
    
        nano_remove_array = - nano_composition_arr - nano_new_aboundance 

    else:
        return np.array([0]*len(nano_composition_arr))

    return nano_remove_array


def make_stoichiometric(structure,nano_remove_array):
   
    from mendeleev import element

    atom_symbols = list(structure.composition.reduced_composition.as_dict().keys())
